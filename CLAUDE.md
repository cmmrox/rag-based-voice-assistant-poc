...
This is a real-time voice assistant powered by OpenAI's Realtime API with RAG (Retrieval-Augmented Generation) capabilities. The system consists of three services:

1. **Frontend** (Next.js 14 + TypeScript) - User interface with WebRTC voice communication
2. **Backend** (FastAPI + Python) - Proxies requests to OpenAI and handles function calling
...
**Critical**: The RAG integration uses OpenAI's function calling, NOT transcription-based parsing:
...
- `constants/tools.ts` - Function definitions (rag_knowledge)
...
- Health check endpoint (checks RAG service connectivity)
